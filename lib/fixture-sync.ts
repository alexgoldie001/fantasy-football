...
export async function syncFixtureScores() {
  ...
  const gameweeks=[...new Set([...fixtureMap.values()].map((fixture:any)=>fixture.event))]; const records:any[]=[];
  for(const gameweek of gameweeks){const response=await fetch(`https://fantasy.premierleague.com/api/event/${gameweek}/live/`,{headers:{'User-Agent':'BailsAndGoldiesFantasy/1.0'},cache:'no-store'});if(!response.ok)continue;const live=await response.json();for(const element of live.elements||[]){for(const explain of (element.explain||[]) as Explain[]){const fixture=fixtureMap.get(explain.fixture);if(!fixture)continue;const stats=new Map((explain.stats||[]).map(stat=>[stat.identifier,stat]));const total=(explain.stats||[]).reduce((sum,stat)=>sum+Number(stat.points||0),0);const bonus=Number(stats.get('bonus')?.points||0);records.push({fpl_id:element.id,fixture_id:explain.fixture,gameweek,kickoff_at:fixture.kickoff_time,points_excluding_bonus:total-bonus,goals:Number(stats.get('goals_scored')?.value||0),assists:Number(stats.get('assists')?.value||0),clean_sheets:Number(stats.get('clean_sheets')?.value||0),raw:explain,updated_at:new Date().toISOString()});}}}
