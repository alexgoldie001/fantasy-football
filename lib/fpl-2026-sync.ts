...
function recordsFromFeed(fpl:any) {
...
export async function addMissingFpl2026Players(fpl:any) {
  const records = recordsFromFeed(fpl);
  const db = supabaseAdmin();
  const { data:stored, error:storedError } = await db.from('fpl_players_2026_27').select('fpl_id').eq('season', '2026/27');
  if (storedError) throw storedError;
  const existingIds = new Set((stored || []).map(player => player.fpl_id));
  const missing = records.filter(player => !existingIds.has(player.fpl_id));
  if (!missing.length) return 0;
  const { error } = await db.from('fpl_players_2026_27').upsert(missing, { onConflict:'fpl_id' });
  if (error) throw error;
  const liveRecords = missing.map(({ season, source_updated_at, ...player }) => player);
  const { error:liveError } = await db.from('fpl_players').upsert(liveRecords, { onConflict:'fpl_id' });
