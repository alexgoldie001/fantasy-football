...
const weeks:Period[] = Array.from({ length:42 }, (_, index) => { const start = new Date(Date.parse('2026-08-18T00:01:00.000Z') + index * 604800000); return { label:`Week ${index + 1}`, start:start.toISOString(), end:new Date(start.getTime() + 604800000).toISOString() }; });
const months:Period[] = ['August','September','October','November','December','January','February','March','April','May'].map((name, index) => { const year = index < 5 ? 2026 : 2027, month = index < 5 ? index + 7 : index - 5; return { label:`${name} ${year}`, start:new Date(Date.UTC(year, month, 1)).toISOString(), end:new Date(Date.UTC(year, month + 1, 1)).toISOString() }; });
...
export async function seasonStats() {
  ...
  const [{ data:memberships, error:membershipsError }, { data:squads, error:squadsError }, { data:players, error:playersError }] = await Promise.all([db.from('squad_players').select('squad_id,fpl_id,acquired_at,released_at'), db.from('squads').select('id,name'), db.from('fpl_players').select('fpl_id,web_name')]);
  ...
  const playerTotal = (field:'goals'|'assists'|'points', position?:string) => { const values = new Map<string, number>(); for (const { stat } of ownedStats) { if (position && stat.position !== position) continue; const id = String(stat.fpl_id); values.set(id, (values.get(id) || 0) + Number(stat[field] || 0)); } return topEntry(values, new Map([...playerNames].map(([id, name]) => [String(id), name]))); };
  const topTeamWeek = teamPeriod(weeks), topTeamMonth = teamPeriod(months), topPlayerWeek = playerPeriod(weeks), topPlayerMonth = playerPeriod(months), topGoals = playerTotal('goals'), topAssists = playerTotal('assists'), topGoalkeeper = playerTotal('points', 'GK'), topDefender = playerTotal('points', 'DEF'), topMidfielder = playerTotal('points', 'MID'), topForward = playerTotal('points', 'FWD');
  ...
  const records:RecordRow[] = [
    ...
    { id:'defender', category:'Highest-scoring defender', subject:topDefender.name, value:`${topDefender.value} pts`, detail:'Only points scored while league-owned', playerId:playerId(topDefender.key) },
    { id:'midfielder', category:'Highest-scoring midfielder', subject:topMidfielder.name, value:`${topMidfielder.value} pts`, detail:'Only points scored while league-owned', playerId:playerId(topMidfielder.key) },
    { id:'forward', category:'Highest-scoring forward', subject:topForward.name, value:`${topForward.value} pts`, detail:'Only points scored while league-owned', playerId:playerId(topForward.key) },
  ];
  return { records };
}
