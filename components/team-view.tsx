...
export function TeamView({ slug = 'north-bank' }:{ slug?:string }) {
  ...
  const [scoreMode, setScoreMode] = useState<'week' | 'season'>('week');
  ...
  useEffect(() => {
    ...
    else if (scoreMode === 'season') query.set('period', 'season');
    else if (week) query.set('week', week);
  ...
  const selectedWeek = scoreMode === 'week' ? (week || team.selectedWeek || '') : '';
  const showingPeriod = scoreMode === 'week' || Boolean(month);
  ...
  const primaryScoreLabel = month ? 'Monthly score' : scoreMode === 'season' ? 'Season score' : 'Weekly score';
  const primaryScore = month || scoreMode === 'week' ? pointsTotal : seasonPoints;
  ...
  const selectWeek = (value:string) => {
    setMonth('');
    if (value === 'season') { setScoreMode('season'); setWeek(''); return; }
    setScoreMode('week');
    setWeek(value);
  };
  const selectMonth = (value:string) => { setMonth(value); if (value) { setScoreMode('week'); setWeek(''); } };
  ...
  return <AppShell>
    ...
    <section className="team-stats">
      ...
      <div className="team-points"><span><small>{primaryScoreLabel}</small><strong>{`${primaryScore} pts`}</strong></span><select aria-label="Select score period" value={month ? '' : scoreMode === 'season' ? 'season' : selectedWeek} onChange={event => selectWeek(event.target.value)}><option value="" disabled>Weekly score</option><option value="season">Season score</option>{(team.weeks || []).map(option => <option key={option.key} value={option.key}>{option.label}</option>)}</select></div>
    ...
    <section className="panel squad-pitch">
      <div className="squad-pitch-head"><div><p className="eyebrow">Squad visual</p><h2>Formation {formation}</h2></div><span>{showingPeriod ? team.pointsLabel : 'Season score'}</span></div>
      <div className="squad-pitch-grass"><div className="pitch-goal" aria-hidden="true"><i/><i/><i/></div><div className="pitch-penalty-box pitch-penalty-box-top" aria-hidden="true"/><div className="pitch-halfway-line" aria-hidden="true"/>{positions.map(position => <div className={`pitch-line pitch-line-${position.toLowerCase()}`} key={position}>{orderedPlayers.filter(player => player.position === position).map((player, index) => <div className="pitch-player-card" key={`${player.name}-${index}`}>{player.teamId ? <img className="club-shirt" src={`/kits/${player.position === 'GK' ? 'gk_' : ''}${player.teamId}.png`} alt={`${player.team} ${player.position === 'GK' ? 'goalkeeper' : 'outfield'} shirt`}/> : <span className="mini-shirt" aria-hidden="true"/>}{player.fplId ? <button className="pitch-player-name" onClick={() => setSelectedPlayerId(player.fplId!)}>{player.name}</button> : <strong>{player.name}</strong>}<small className="pitch-player-points">{player.points} pts</small></div>)}</div>)}<div className="pitch-penalty-box pitch-penalty-box-bottom" aria-hidden="true"/><div className="pitch-bottom-arc" aria-hidden="true"/></div>
    </section>
    <section className="panel squad-list"><div className="squad-list-head"><span>Player</span><span>Club</span><span>Position</span><span>{showingPeriod ? 'Period points' : 'Season score'}</span><span>Purchase price</span></div>{orderedPlayers.map((player, index) => <div className="squad-list-row" key={`${player.name}-${index}`}><div><strong>{player.fplId ? <button className="player-stat-link" onClick={() => setSelectedPlayerId(player.fplId!)}>{player.name}</button> : player.name}</strong></div><span data-label="Club">{player.team}</span><span data-label="Position"><i className={`position-chip ${player.position.toLowerCase()}`}>{player.position}</i></span><strong data-label={showingPeriod ? 'Period points' : 'Season score'}>{player.points}</strong><strong data-label="Purchase price">{typeof player.price === 'string' ? player.price.split(' / ').map(price => `£${(Number(price) / 10).toFixed(1)}m`).join(' / ') : `£${(player.price / 10).toFixed(1)}m`}</strong></div>)}</section>
