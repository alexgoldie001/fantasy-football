...
export function TeamView({ slug = 'north-bank' }:{ slug?:string }) {
  ...
  return <AppShell>
    ...
    <section className="panel squad-list"><div className="squad-list-head"><span>Player</span><span>Club</span><span>Position</span><span>{showingWeek ? 'Week points' : 'Season points'}</span><span>Purchase price</span></div>{orderedPlayers.map((player, index) => <div className="squad-list-row" key={`${player.name}-${index}`}><div><strong>{player.name}</strong><small>Purchased player</small></div><span>{player.team}</span><span className={`position-chip ${player.position.toLowerCase()}`}>{player.position}</span><strong>{player.points}</strong><strong>{typeof player.price === 'string' ? player.price.split(' / ').map(price => `£${(Number(price) / 10).toFixed(1)}m`).join(' / ') : `£${(player.price / 10).toFixed(1)}m`}</strong></div>)}</section>
    <section className="panel squad-pitch">
      ...
      <div className="squad-pitch-grass">
        <div className="pitch-goal" aria-hidden="true"><i/><i/><i/></div>
        <div className="pitch-penalty-box pitch-penalty-box-top" aria-hidden="true"/>
        <div className="pitch-halfway-line" aria-hidden="true"/>
        {positions.map(position => <div className={`pitch-line pitch-line-${position.toLowerCase()}`} key={position}>{orderedPlayers.filter(player => player.position === position).map((player, index) => <div className="pitch-player-card" key={`${player.name}-${index}`}>{player.teamId ? <img className="club-shirt" src={`/kits/${player.position === 'GK' ? 'gk_' : ''}${player.teamId}.webp`} alt={`${player.team} ${player.position === 'GK' ? 'goalkeeper' : 'outfield'} shirt`}/> : <span className="mini-shirt" aria-hidden="true"/>}<strong>{player.name}</strong><small>{player.points} pts</small></div>)}</div>)}
        <div className="pitch-penalty-box pitch-penalty-box-bottom" aria-hidden="true"/>
