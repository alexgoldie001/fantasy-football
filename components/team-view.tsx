...
export function TeamView({ slug = 'north-bank' }:{ slug?:string }) {
  ...
  return <AppShell>
    ...
    <section className="team-stats">
      ...
      <div className="team-points"><span><small>{primaryScoreLabel}</small><strong>{`${primaryScore} pts`}</strong></span><select aria-label="Select score period" value={month ? '' : scoreMode === 'season' ? 'season' : selectedWeek} onChange={event => selectWeek(event.target.value)}><option value="" disabled>- -</option><option value="season">Season score</option>{(team.weeks || []).map(option => <option key={option.key} value={option.key}>{option.label}</option>)}</select></div>
