...
export function TeamView({ slug = 'north-bank' }: { slug?: string }) {
  ...
  useEffect(()=>{fetch(`/api/teams/${slug}`).then(async response=>{if(!response.ok)throw new Error('Team not found.');return response.json();}).then(data=>{setTeam(data);setLoadError('');}).catch(error=>{if(slug !== 'north-bank')setLoadError(error.message);});},[slug]);
