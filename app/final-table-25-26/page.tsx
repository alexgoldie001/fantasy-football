...
export default function FinalTable2526Page() {
  const [rows, setRows] = useState<Row[]>([]), [error, setError] = useState('');
  useEffect(() => { fetch('/api/final-table', { cache:'no-store' }).then(async response => { const result = await response.json(); if (!response.ok) throw new Error(result.error); setRows([...((result.rows || []) as Row[])].sort((a, b) => b.total - a.total)); }).catch(reason => setError(reason.message || 'Unable to load the archived table.')); }, []);
