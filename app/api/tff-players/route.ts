...
export async function GET() {
  try {
    ...
    return NextResponse.json({ season:'2025/26', players:(players || []).map((player:any) => {
      ...
      return { id:player.tff_id, name:player.display_name, team:player.team_name, teamCode:player.team_code || '', position:player.position, price:player.current_price, points:value(raw, 'total_points'), weeklyScores:Object.fromEntries(Object.entries(roundScores).map(([round, score]) => [round, Number(score || 0)])), starts:value(raw, 'starting11'), cleanSheets:value(raw, 'full_clean_sheets'), partialCleanSheets:value(raw, 'partial_clean_sheets'), assists:value(raw, 'assists'), goals:value(raw, 'goals'), tackles:value(raw, 'tackles'), saves:value(raw, 'saves'), goalsConceded:value(raw, 'goals_conceded'), yellowCards:value(raw, 'yellow_cards'), redCards:value(raw, 'red_cards'), penaltiesMissed:value(raw, 'missed_pen'), penaltiesSaved:value(raw, 'saved_pen') };
    }) });
