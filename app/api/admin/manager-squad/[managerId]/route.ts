...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ managerId:string }> }) {
  ...
  try { const db=supabaseAdmin(); const {data:squad,error:squadError}=await db.from('squads').select('id,league_id,budget').eq('manager_id',managerId).single(); if(squadError||!squad)throw squadError||new Error('Squad not found.'); const hasReplacement=changes.some(change=>Boolean(change.replacementFplId)); let rules:any=null; let currentEvent:number|null=null; let liveStats=new Map<number,Record<string,number>>();
    if(hasReplacement){const rulesResult=await db.from('scoring_rules').select('*').eq('league_id',squad.league_id).eq('is_active',true).maybeSingle();rules=rulesResult.data;const bootstrapResponse=await fetch('https://fantasy.premierleague.com/api/bootstrap-static/',{cache:'no-store'});if(!bootstrapResponse.ok)throw new Error('Unable to read the FPL gameweek.');const bootstrap=await bootstrapResponse.json();currentEvent=bootstrap.events.find((event:any)=>event.is_current)?.id || bootstrap.events.find((event:any)=>event.is_next)?.id || 1;const liveResponse=await fetch(`https://fantasy.premierleague.com/api/event/${currentEvent}/live/`,{cache:'no-store'});if(!liveResponse.ok)throw new Error('Unable to read live FPL scores.');const live=await liveResponse.json();liveStats=new Map(live.elements.map((row:any)=>[row.id,row.stats]));}
