...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ managerId:string }> }) {
  ...
  try { const db=supabaseAdmin(); const {data:squad,error:squadError}=await db.from('squads').select('id,league_id,budget').eq('manager_id',managerId).single(); if(squadError||!squad)throw squadError||new Error('Squad not found.'); const hasReplacement=changes.some(change=>Boolean(change.replacementFplId)); let rules:any=null; let currentEvent:number|null=null; let liveStats=new Map<number,Record<string,number>>();
    if(hasReplacement){const rulesResult=await db.from('scoring_rules').select('*').eq('league_id',squad.league_id).eq('is_active',true).maybeSingle();rules=rulesResult.data;try{const bootstrapResponse=await fetch('https://fantasy.premierleague.com/api/bootstrap-static/',{cache:'no-store'});if(bootstrapResponse.ok){const bootstrap=await bootstrapResponse.json();currentEvent=bootstrap.events.find((event:any)=>event.is_current)?.id || bootstrap.events.find((event:any)=>event.is_next)?.id || null;if(currentEvent){const liveResponse=await fetch(`https://fantasy.premierleague.com/api/event/${currentEvent}/live/`,{cache:'no-store'});if(liveResponse.ok){const live=await liveResponse.json();liveStats=new Map(live.elements.map((row:any)=>[row.id,row.stats]));}}}}catch{/* Pre-gameweek swaps correctly start from zero. */}}
  ...
  }catch(error){return NextResponse.json({error:error instanceof Error?`Unable to save squad amendments: ${error.message}`:'Unable to save squad amendments.'},{status:500});}
}
