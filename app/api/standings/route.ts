...
export async function GET() {
  try {
    ...
    const standings = (squads || []).map(squad => ({ id: squad.id, name: squad.name, manager: profileNames.get(squad.manager_id) || 'Manager', points: (playersBySquad.get(squad.id) || []).reduce((total, points) => total + points, 0), week: 0 })).sort((a, b) => b.points - a.points || a.name.localeCompare(b.name)).map((row, index) => ({ ...row, rank: index + 1, change: '—' }));
