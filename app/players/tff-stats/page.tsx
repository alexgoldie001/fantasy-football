...
type Player = { id:number; name:string; team:string; teamCode:string; position:string; price:number; points:number; weeklyScores:Record<string, number>; starts:number; cleanSheets:number; partialCleanSheets:number; assists:number; goals:number; tackles:number; saves:number; goalsConceded:number; yellowCards:number; redCards:number; penaltiesMissed:number; penaltiesSaved:number };
...
export default function TffStatsPage() {
  const [players, setPlayers] = useState<Player[]>([]), [loading, setLoading] = useState(true), [error, setError] = useState(''), [search, setSearch] = useState(''), [position, setPosition] = useState('All'), [club, setClub] = useState('All'), [week, setWeek] = useState('season'), [sort, setSort] = useState<keyof Player>('points');
  ...
  const scoreForWeek = (player:Player) => week === 'season' ? player.points : player.weeklyScores[week] || 0;
  const filtered = useMemo(() => players.filter(player => (position === 'All' || player.position === position) && (club === 'All' || player.team === club) && `${player.name} ${player.team}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => (sort === 'position' ? positionOrder[a.position] - positionOrder[b.position] : sort === 'points' ? scoreForWeek(b) - scoreForWeek(a) : Number(b[sort]) - Number(a[sort])) || a.name.localeCompare(b.name)), [players, search, position, club, week, sort]);
  return <AppShell>
