...
const columns: { key:keyof Player; label:string }[] = [
  { key:'goals', label:'G' }, { key:'assists', label:'A' }, { key:'cleanSheets', label:'CS' }, { key:'defensiveContribution', label:'Def con.' }, { key:'yellowCards', label:'YC' }, { key:'redCards', label:'RC' }, { key:'penaltiesMissed', label:'Pen miss' }, { key:'penaltiesSaved', label:'Pen saved' },
];
const positionOrder: Record<string, number> = { GK:1, DEF:2, MID:3, FWD:4 };

export default function Players202627Page() {
  ...
  useEffect(() => { fetch('/api/players/2026-27').then(async response => { const data = await response.json(); if (!response.ok) throw new Error(data.error); setPlayers(data.players || []); }).catch(reason => setError(reason.message || 'Unable to load the 2026/27 FPL player list.')).finally(() => setLoading(false)); }, []);
  ...
  return <AppShell>
    <section className="page-heading"><p className="eyebrow">Official FPL data · 2026/27</p><h1>2026/27 player market.</h1><Link className="excel-link" href="/players">Back to 2025/26 players</Link></section>
    <section className="filters"><label><Search size={18}/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search players or teams"/></label><div className="selects"><select value={position} onChange={event => setPosition(event.target.value)}><option value="All">All Positions</option><option>GK</option><option>DEF</option><option>MID</option><option>FWD</option></select><select value={club} onChange={event => setClub(event.target.value)}><option value="All">All Teams</option>{clubs.map(team => <option key={team}>{team}</option>)}</select><select value={ownership} onChange={event => setOwnership(event.target.value)}><option value="All">All Ownership</option><option>Available</option><option>Owned</option></select></div></section>
    <section className="panel player-table player-market">{loading ? <p className="table-message">Loading the official 2026/27 FPL player list…</p> : error ? <p className="form-error">{error}</p> : <div className="market-scroll"><div className="market-head"><span>Player</span><button onClick={() => setSort('position')} className={sort === 'position' ? 'sorted' : ''}>Pos.</button><span>Owner</span><button onClick={() => setSort('points')} className={sort === 'points' ? 'sorted' : ''}>Pts</button><span>FPL price</span>{columns.map(column => <button key={column.key} onClick={() => setSort(column.key)} className={sort === column.key ? 'sorted' : ''}>{column.label}</button>)}</div>{filtered.map(player => <div className="market-row" key={player.id}><div><strong>{player.name}</strong><small>{player.team}</small></div><span className={`position-chip ${player.position.toLowerCase()}`}>{player.position}</span><span className="available">Available</span><strong>{player.points}</strong><span>£{(player.price / 10).toFixed(1)}m</span>{columns.map(column => <strong key={column.key}>{player[column.key] as number}</strong>)}</div>)}</div>}</section>
  </AppShell>;
}
